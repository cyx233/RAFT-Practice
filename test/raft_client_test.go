...
// A creates and syncs with a file. B syncs. B updates and syncs the file. A syncs again.
func TestSyncTwoClientsUpdateFileLeaderFailure(t *testing.T) {
	t.Logf("client1 syncs with file1. client2 syncs with file1 (different content). client1 syncs again.")
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath)
	defer EndTest(test)
	...
	//check syncs
	if c := DirFullySynced(*worker1, *worker2); !c {
		t.Fatalf("Files are not the same between clients")
	}

	//client2 update files
...
func TestSyncTwoClientsUpdateLargeFileLeaderFailure(t *testing.T) {
	t.Logf("client1 syncs with file1. client2 syncs with file1 (different content). client1 syncs again.")
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath)
	defer EndTest(test)
...
// A creates and syncs with a file. B syncs. A delete the files. A syncs again. B syncs again
func TestSyncTwoClientsDeleteFileLeaderFailure(t *testing.T) {
	t.Logf("client1 syncs with file1. client2 syncs with file1 (different content). client1 syncs again.")
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath)
	defer EndTest(test)
	...
	//client1 delete the file
	worker1.DeleteFile(file1)
	...
	if fileMeta1 == nil || !IsTombHashList(fileMeta1[file1].BlockHashList) {
		t.Fatalf("Wrong BlockHash for deleted file1 in client1 metadata.")
	}

	if _, exist := worker1.ListAllFile()[file1]; exist {
		t.Fatalf("File is not deleted in client1's folder")
	}
	...
	if fileMeta2 == nil || !IsTombHashList(fileMeta2[file1].BlockHashList) {
		t.Fatalf("Wrong BlockHash for deleted file1 in client2 metadata.")
	}

	if _, exist := worker2.ListAllFile()[file1]; exist {
		t.Fatalf("File is not deleted in client2's folder")
	}
}

func TestTooManyFailure(t *testing.T) {
	t.Logf("client1 syncs with file1. client2 syncs with file1 (different content). client1 syncs again.")
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath)
	defer EndTest(test)
	...
	test.Clients[1].Crash(test.Context, &emptypb.Empty{})
	test.Clients[2].SetLeader(test.Context, &emptypb.Empty{})
	succ, err := test.Clients[2].SendHeartbeat(test.Context, &emptypb.Empty{})
	if succ.GetFlag() {
		t.Fatalf("Succuss with too few servers!")
	}
	log.Println(err)
}
